...
function sanitizeLocalidadInput(
  ...
  if (
    ...
    req.body.sanitizeLocalidadInput.codigo_localidad.trim().length === 0
  ) {
  ...
  if (
    req.body.sanitizeLocalidadInput.codigo_localidad &&
    !/^[0-9]+$/.test(req.body.sanitizeLocalidadInput.codigo_localidad)
  ) {
  ...
  if (
    ...
    req.body.sanitizeLocalidadInput.nombre_localidad.trim().length === 0
  ) {
...
async function findAll(req: Request, res: Response) {
  try {
    ...
    const localidades = await em.find(Localidad, filter, {
      populate: ['zonas'],
    });
    res
      .status(200)
...
async function update(req: Request, res: Response) {
  try {
    ...
    if (
      req.body.sanitizeLocalidadInput.codigo_localidad &&
      req.body.sanitizeLocalidadInput.codigo_localidad !==
        localidadToUpdate.codigo_localidad
    ) {
...
async function remove(req: Request, res: Response) {
  try {
    ...
    const localidadToDelete = await em.findOneOrFail(Localidad, idLoc, {
      populate: ['zonas'],
    });
    ...
    for (const zona of localidadToDelete.zonas) {
      ...
      if (usuariosCount > 0) {
        res
          ...
          .json({
            message:
              'No se puede eliminar la zona porque tiene usuarios asociados a sus zonas.',
          });
        return;
      }
      ...
      if (pedidosCount > 0) {
        res
          ...
          .json({
            message:
              'No se puede eliminar la zona porque tiene pedidos de resolución asociados a sus zonas.',
          });
        return;
      }
    }
...
async function findLocalidadByName(nombre_localidad: string) {
  try {
    const localidadFound = await em.findOneOrFail(Localidad, {
