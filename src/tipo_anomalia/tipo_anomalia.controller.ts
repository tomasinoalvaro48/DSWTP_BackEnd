...
function sanitizeTipoInput(req: Request, res: Response, next: NextFunction) {
  ...
  if (
    !req.body.sanitizeTipoInput.nombre_tipo_anomalia ||
    req.body.sanitizeTipoInput.nombre_tipo_anomalia.trim().length === 0
  ) {
  ...
  if (
    req.body.sanitizeTipoInput.dificultad_tipo_anomalia &&
    !/^[1-3]$/.test(req.body.sanitizeTipoInput.dificultad_tipo_anomalia)
  ) {
  ...
  Object.keys(req.body.sanitizeTipoInput).forEach((key) => {
    if (req.body.sanitizeTipoInput[key] === undefined)
      delete req.body.sanitizeTipoInput[key];
  });
  next();
}
...
async function findAll(req: Request, res: Response) {
  try {
    ...
    res
      .status(200)
      .json({ message: 'found all tipos de anomalia', data: tipos });
  } catch (error: any) {
...
async function add(req: Request, res: Response) {
  try {
    ...
    if (await validateName(nombre_tipo)) {
      res
        .status(409)
        .json({ message: 'nombre de tipo duplicado', data: nombre_tipo });
    } else {
...
async function update(req: Request, res: Response) {
  try {
    ...
    if (nombre_tipo && (await validateName(nombre_tipo))) {
      res
        .status(409)
        .json({ message: 'nombre de tipo duplicado', data: tipoToUpdate });
    } else {
      em.assign(tipoToUpdate, req.body.sanitizeTipoInput);
      await em.flush();
      res
        .status(200)
        .json({ message: 'tipo de anomalia updated', data: tipoToUpdate });
    }
  } catch (error: any) {
...
async function validateName(nombre_tipo: string) {
  const tipo_nombre = await em.find(Tipo_Anomalia, {
    nombre_tipo_anomalia: nombre_tipo,
  });
