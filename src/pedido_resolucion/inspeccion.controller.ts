...
function sanitizeInspeccionInput(req: Request, res: Response, next: NextFunction) {
...
async function findAll(req: Request, res: Response) {
  try {
    ...
    console.log(`Error al obtener las inspecciones: ${error.message}`)
...
async function addInspeccion(req: Request, res: Response) {
  try {
    ...
    await em.flush()
    res.status(200).json({ message: 'Inspección agregada', data: newInspeccion })
  } catch (error: any) {
    console.log(`Error al agregar inspección: ${error.message}`)
    res.status(500).json({ message: error.message })
  }
}
...
async function remove(req: Request, res: Response) {
  try {
    ...
    res.status(200).json({ message: 'Remove inspeccion', data: inspeccionToDelete })
  } catch (error: any) {
    console.log(`Error al eliminar la inspección: ${error.message}`)
    res.status(500).json({ message: error.message })
  }
}
