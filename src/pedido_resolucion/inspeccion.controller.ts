...
async function addInspeccion(req: Request, res: Response) {
  try {
    req.body.sanitizeInspeccionInput.numero_inspeccion =
      await nroProximaInspeccion(
        req.body.sanitizeInspeccionInput.pedido_resolucion
      );

    const newInspeccion = em.create(
      Inspeccion,
      req.body.sanitizeInspeccionInput
    );
    await em.flush();
...
async function nroProximaInspeccion(pedido_resolucion: ObjectId) {
  try {
    const ultima = await em.findOneOrFail(
      Inspeccion,
      { pedido_resolucion },
      { orderBy: { numero_inspeccion: 'desc' } }
    );
